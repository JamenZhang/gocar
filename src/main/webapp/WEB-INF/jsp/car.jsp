        ...
        <form id = "editForm"  method = "post" enctype="multipart/form-data">
            <table >
                ...
                <tr class="mTr">
                    <td>图片</td>
                    <td>
                        <input class="easyui-filebox"  name="carIcon" style="width:220px">
                    </td>
                ...
                <tr class="mTr">
                    ...
                    <td>
                        <input id="bCid" class="easyui-combobox" name="bCid"
                               data-options="valueField:'cid',textField:'cName',url:'car/loadCategory',required:true,editable:false" />
                    </td>
                </tr>
                <tr>
                    <td>数量：</td>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#updateBtn").click(function () {
            ...
            var car = $('#dg').datagrid('getSelected');
            if(!car){
                $.messager.alert('错误提醒','请选择一个单车！','info');
                return false;
            }
            $("#editForm").form("load","car/loadForm?bid="+car.bid);
        ...
        $('#removeBtn').bind('click', function(){
            //拿到一个json的数组对象
            var cars = $('#dg').datagrid('getSelections');
            ...
            for (var i = 0; i < cars.length; i++) {
                bids.push(cars[i].bid);
                cids.push(cars[i].bCid);
            }
            bids.join(",");cids.join(",");

            $.messager.confirm('确认','您确认想删除该单车吗？',function(r){
                if (r){
                    $.post('car/remove',{"bids":bids.toString(),"cids":cids.toString()},function (data) {
                        $.messager.show({
                            title:'消息',
                            msg:data.message,
                            timeout:3000,
